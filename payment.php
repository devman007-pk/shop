...
// ดึงข้อมูลที่อยู่ (ถ้ามีข้อมูล Tax ให้ใช้ Tax ถ้าไม่มีให้ใช้ที่อยู่ User)
...
// Fallback: ถ้ายังไม่มีสินค้าจริงในฐานข้อมูล
if (empty($orderItems)) {
...
// =========================================================================
// จัดการการอัปโหลดสลิป แจ้งเตือนอีเมล และ Redirect 
// =========================================================================
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['payment_slip'])) {
    ...
    if ($file['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($ext, $allowed)) {
            ...
            $newFileName = 'slip_order_' . $orderId . '_' . time() . '.' . $ext;
            $destination = $uploadDir . $newFileName;
            
            if (move_uploaded_file($file['tmp_name'], $destination)) {
                $update = $pdo->prepare("UPDATE orders SET status = 'payment_review', payment_slip = ? WHERE id = ?");
                $update->execute([$newFileName, $orderId]);
                
                // สร้างเนื้อหา Email
                $toEmail = $userData['email'];
                $subject = "ใบเสนอราคา / ยืนยันการสั่งซื้อเลขที่ #" . $order['order_number'];
                ...
                foreach($orderItems as $item) {
                    ...
                    $itemsHtml .= '
                    <tr>
                        <td align="center" style="border-bottom:1px solid #eee; padding:10px;">'.$i++.'</td>
                        <td style="border-bottom:1px solid #eee; padding:10px;">'.h($item['name']).'</td>
                        <td align="right" style="border-bottom:1px solid #eee; padding:10px;">'.number_format((float)$item['quantity'], 2).'</td>
                        <td align="right" style="border-bottom:1px solid #eee; padding:10px;">'.number_format((float)$item['unit_price'], 2).'</td>
                        <td align="right" style="border-bottom:1px solid #eee; padding:10px;">'.number_format((float)$itemTotal, 2).' ฿</td>
                    </tr>';
                }
                ...
                <body style="font-family: Arial, sans-serif; background: #f4f4f4; padding: 20px;">
                    <div style="max-width: 700px; margin: 0 auto; background: #fff; padding: 30px; border-radius: 8px;">
                        <h2 style="color: #1ba2b4; border-bottom: 2px solid #1ba2b4; padding-bottom: 10px;">ใบเสนอราคา / คำสั่งซื้อ</h2>
                        
                        <p><strong>บริษัท ออน ไทม์ แมนเนจเม้นท์ จำกัด (ON TIME MANAGEMENT CO.,LTD.)</strong><br>
                        50/123 หมู่7 ตำบลเนินพระ อำเภอเมืองระยอง จ.ระยอง 21000<br>
                        เลขประจำตัวผู้เสียภาษี : 0215562008121</p>
                        ...
                        <table width="100%" cellspacing="0" cellpadding="0" style="margin-top: 20px; font-size: 14px;">
                            ...
                            '.$itemsHtml.'
                            <tr>
                                <td align="center" style="border-bottom:2px solid #000; padding:10px;">'.$i.'</td>
                                <td style="border-bottom:2px solid #000; padding:10px;">การจัดส่งมาตรฐาน (ฟรี)</td>
                                <td align="right" style="border-bottom:2px solid #000; padding:10px;">1.00</td>
                                <td align="right" style="border-bottom:2px solid #000; padding:10px;">0.00</td>
                                <td align="right" style="border-bottom:2px solid #000; padding:10px;">0.00 ฿</td>
                            </tr>
                        </table>
                ...
                $headers .= "From: no-reply@otm.co.th" . "\r\n";
            ...
            $error = "อนุญาตเฉพาะไฟล์รูปภาพ (JPG, PNG) เท่านั้น";
        }
    } else {
...
<head>
  ...
  <style>
    ...
    @media print {
        ...
        .print-company-info { text-align: left; width: 450px; font-size: 0.9rem; line-height: 1.5; }
        ...
        if (fileInput && fileMsg && dropArea) {
            fileInput.addEventListener('change', function() {
                if (this.files && this.files.length > 0) {
                    ...
                    // สร้าง FileReader เพื่ออ่านรูปภาพและโชว์พรีวิว
